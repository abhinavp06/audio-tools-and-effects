...
void Limiter::prepare(int sr, int channels) {
  ...
  double attack_s = 0.1 / 1000.0;
  alpha_attack = 1 - std::exp(-1 / (attack_s * sample_rate));

  for (int i = 0; i < channel_count; i++) {
    envelope_followers[i].setAttack(0.1);
  }
}

void Limiter::setThreshold(double threshold_db) { threshold = threshold_db; }

void Limiter::setRelease(double release_ms) {
  for (int i = 0; i < channel_count; i++) {
...
void Limiter::setMakeupGain(double gain_db) { makeup_gain = gain_db; }

void Limiter::apply(AudioBuffer &buffer) {
  ...
  for (int ch = 0; ch < buffer.channels; ch++) {
    for (size_t i = ch; i < buffer.samples.size(); i += buffer.channels) {
      ...
      if (input_db - threshold >= 0) {
        gain_reduction = (input_db - threshold);
      }
      ...
      if (gain_reduction > gain_reduction_smoother_history[ch]) {
        gain_reduction_smoother_history[ch] =
            ...
            one_minus_alpha_attack * gain_reduction;
      } else {
        gain_reduction_smoother_history[ch] =
      ...
      sample = std::clamp<double>(
          sample * std::pow(10, (((-1 * gain_reduction_smoother_history[ch]) +
                                  makeup_gain) /
                                 20)),
          -1, 1);
    }
